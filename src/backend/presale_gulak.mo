// ============================================================
// GULAK Governance Token Pre-Sale Canister
...
actor PresaleGulak {
  ...
  let PRICE_USD_CENTS         : Nat = 100;        // PLACEHOLDER: $1.00 per token (in cents)
  let TOTAL_ALLOCATION        : Nat = 1_000_000;  // PLACEHOLDER: total GULAK tokens in pre-sale pool
  let MIN_PURCHASE_USD_CENTS  : Nat = 2_500;      // PLACEHOLDER: $25.00 minimum purchase (in cents)
  let MAX_PURCHASE_USD_CENTS  : Nat = 50_000;     // PLACEHOLDER: $500.00 maximum purchase (in cents)
  ...
  let VESTING_IMMEDIATE_PCT   : Nat = 25;         // PLACEHOLDER: 25 percent released at TGE
  let VESTING_CLIFF_MONTHS    : Nat = 12;         // PLACEHOLDER: remaining 75 percent vests over 12 months
  // PLACEHOLDER: minimum raise for pre-sale to succeed ($50,000 = 5_000_000 cents)
  let MIN_RAISE_USD_CENTS     : Nat = 5_000_000;
  ...
  // PLACEHOLDER: wallet spending threshold above which whale rate applies ($250 = 25_000 cents)
  let WHALE_THRESHOLD_CENTS   : Nat = 25_000;
  ...
  public type ReceiptRecord = {
    receiptId               : Text;
    buyer                   : Principal;
    tokenType               : Text;    // "GULAK"
    amountPurchased         : Nat;     // number of GULAK tokens
    purchaseTimestamp       : Int;
    vestingImmediatePercent : Nat;
    vestingCliffDate        : Int;     // nanosecond timestamp when cliff ends
    refundable              : Bool;
  };
  ...
  /// Purchase GULAK tokens.
  /// amountUsdCents is the purchase amount expressed in USD cents (e.g. 2500 = $25.00).
  ...
  public shared ({ caller }) func purchase(amountUsdCents : Nat) : async Result.Result<Text, Text> {
    ...
    let receiptId : Text = "GULAK-" # state.receiptCounter.toText();
    // Calculate vesting cliff timestamp (12 months = 12 * 30 * 24 * 3600 * 1_000_000_000 ns)
    // PLACEHOLDER: cliff duration assumes 30-day months
    let cliffNs : Int = Int.fromNat(VESTING_CLIFF_MONTHS * 30 * 24 * 3600) * 1_000_000_000;
    let vestingCliffDate : Int = now + cliffNs;
    let receipt : ReceiptRecord = {
      ...
      vestingCliffDate        = vestingCliffDate;
      refundable              = false;
    };
